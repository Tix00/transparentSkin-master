...
def skin_detector_bgr(bgr_image):
    ...
    # We create the result image with black background
    # result = np.zeros((h, w, 1), dtype="uint8")
    # facciamolo con sfondo nero a 3 valori
    ...
    for y in range(0, h):
        for x in range(0, w):
            (b, g, r) = bgr_image[y, x]
            if bgr_skin(b, g, r):
                #rendiamo l'immagine rossa nei punti di pelle
                result[y, x] = [0, 0, 255]
                #result[y, x] = 255
...
def skinSegmentation(image):
    # utilizziamo BRG come metodo
    ...
    #image = apply_skin_detector(image, 'bgr')
    image = skin_detector_bgr(image)
